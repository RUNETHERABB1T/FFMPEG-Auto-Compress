...
for %%a in ("*.avif")  do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.avif"
for %%a in ("*.bmp")   do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.bmp"
for %%a in ("*.bpg")   do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.bpg"
for %%a in ("*.gif")   do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.gif"
for %%a in ("*.heif")  do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.heif"
for %%a in ("*.img")   do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.img"
for %%a in ("*.jpg")   do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.jpg"
for %%a in ("*.jpeg")  do ffmpeg -y -i "%%a" -map 0 -c:v libsvtav1 -crf 30 "compressed/%%~na.jpeg"
